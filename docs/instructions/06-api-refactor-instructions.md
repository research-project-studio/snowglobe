# WebMap Archiver: API Layer Refactor Instructions

## For Claude Code

This document provides instructions for creating a clean public API layer in the webmap-archiver CLI package, enabling consistent reuse across Modal, Puppeteer, and other consumers.
...
### Current Problem
- `modal_app.py` reimplements logic that exists in the CLI package
- Layer discovery, archive building, and viewer generation are duplicated
- Bug fixes must be applied in multiple places
- Risk of divergence between CLI and Modal behavior

### Goal
Create a single `api.py` module that:
1. Exposes clean, high-level functions for archive creation
2. Handles all the complexity internally (parsing, processing, layer discovery, packaging)
3. Is the single source of truth used by CLI, Modal, and future consumers

---

## 2. New File: `api.py`

Create `cli/src/webmap_archiver/api.py`:
...
## 3. Update `layer_inspector.py`

Ensure `cli/src/webmap_archiver/tiles/layer_inspector.py` exists and exports these functions:

```python
"""
Layer inspection via protobuf parsing.

Extracts source-layer names directly from MVT tile content.
"""
...
def extract_layer_names_protobuf(tile_content: bytes) -> list[str]:
    ...
    MVT format:
    - Field 3 = layer (repeated, length-delimited)
    - Within layer: Field 1 = name (string, length-delimited)
    ...
    # Decompress if gzipped
    try:
        if tile_content[:2] == b'\x1f\x8b':
            tile_content = gzip.decompress(tile_content)
    except Exception:
        pass
    ...
    return _parse_mvt_layers(tile_content)
...
def _parse_mvt_layers(data: bytes) -> list[str]:
    """Parse MVT protobuf and extract layer names."""
    layer_names = []
    pos = 0
    ...
    try:
        while pos < len(data):
            ...
            if wire_type == 0:
                ...
                if field_number == 3:  # Layer message
                    layer_data = data[pos:pos + length]
                    name = _extract_layer_name(layer_data)
...
def _extract_layer_name(layer_data: bytes) -> str | None:
    """Extract name field from layer submessage."""
    pos = 0
    ...
    try:
        while pos < len(layer_data):
            ...
            if wire_type == 0:
                _, pos = _read_varint(layer_data, pos)
            elif wire_type == 1:
                pos += 8
            elif wire_type == 2:
                length, pos = _read_varint(layer_data, pos)
                
                if field_number == 1:  # Name field
                    return layer_data[pos:pos + length].decode('utf-8')
...
Update `cli/src/webmap_archiver/__init__.py` to export the public API:
...
## 5. Update CLI to Use API

Update `cli/src/webmap_archiver/cli.py` to use the API:

```python
# In the create command, replace direct implementation with:

from .api import create_archive_from_har

@click.command()
@click.argument('har_file', type=click.Path(exists=True))
@click.option('-o', '--output', type=click.Path(), help='Output ZIP path')
@click.option('-n', '--name', help='Archive name')
@click.option('-v', '--verbose', is_flag=True, help='Verbose output')
@click.option('--mode', type=click.Choice(['standalone', 'original', 'full']), default='standalone')
def create(har_file, output, name, verbose, mode):
    """Create archive from HAR file."""
    from pathlib import Path
    
    har_path = Path(har_file)
    
    if not output:
        output = har_path.stem + '.zip'
    ...
    click.echo(f"Created archive: {result.output_path}")
    click.echo(f"  Size: {result.size:,} bytes")
    click.echo(f"  Tiles: {result.tile_count}")
    click.echo(f"  Sources: {len(result.tile_sources)}")


# Add new process command for capture bundles:

@click.command()
@click.argument('bundle_file', type=click.Path(exists=True))
@click.option('-o', '--output', type=click.Path(), help='Output ZIP path')
@click.option('-v', '--verbose', is_flag=True, help='Verbose output')
def process(bundle_file, output, verbose):
    """Process a capture bundle into an archive."""
    from pathlib import Path
    import json
    
    from .api import create_archive_from_bundle
...
2. **Test API directly:**
   ```python
   from webmap_archiver import create_archive_from_bundle, inspect_bundle
   
   # Load a test bundle
   import json
   with open("test_bundle.json") as f:
       bundle = json.load(f)
   ...
   # Create archive
   result = create_archive_from_bundle(bundle, Path("test.zip"), verbose=True)
   print(f"Created: {result.output_path}")
   ```

3. **Test CLI still works:**
   ```bash
   webmap-archive create test.har -o test.zip -v
   ```
...
This refactor:
1. Creates `api.py` as the single source of truth
2. Exposes clean, documented functions
3. Includes layer discovery logic
4. Updates CLI to use the API
5. Makes the package ready for Modal and other consumers

Modal will then simply do:
```python
from webmap_archiver import create_archive_from_bundle

result = create_archive_from_bundle(bundle, output_path)
```