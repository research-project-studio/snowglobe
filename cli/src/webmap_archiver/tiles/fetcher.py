...
class CoverageReport:
    ...
    required_by_zoom: dict[int, int] = field(
        default_factory=dict
    )  # zoom -> count needed for full coverage
...
class CoverageAnalyzer:
    ...
    def analyze(self, captured_tiles: set[TileCoord], expand_zoom: int = 0) -> CoverageReport:
        """
        ...
        report = CoverageReport(bounds=self.bounds, zoom_levels=zoom_levels)
    ...
    def find_missing_tiles(
        self, captured_tiles: set[TileCoord], zoom_levels: list[int]
...
class TileFetcher:
    ...
    def __init__(
        ...
        if not AIOHTTP_AVAILABLE:
            raise ImportError(
                "aiohttp is required for tile fetching. " "Install with: pip install aiohttp"
            )
    ...
    async def fetch_tile(
        self, session: "aiohttp.ClientSession", url: str, coord: TileCoord
    ) -> FetchResult:
            ...
            try:
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        return FetchResult(coord=coord, content=content, status=200)
                ...
                if attempt < self.max_retries:
                    await asyncio.sleep(1.0 * (attempt + 1))
                    continue
                return FetchResult(coord=coord, content=None, error="Timeout")
            except Exception as e:
                if attempt < self.max_retries:
                    await asyncio.sleep(1.0 * (attempt + 1))
                    continue
                return FetchResult(coord=coord, content=None, error=str(e))
...
async def expand_coverage_async(
    ...
    if captured_tiles:
        ...
        print(
            f"    Target zoom range: z{min_zoom}-z{target_max_zoom} (expand by {expand_zoom})",
            flush=True,
        )
    ...
    results = await fetcher.fetch_tiles(url_template, all_missing, progress_callback=fetch_progress)
...
def analyze_coverage(
    captured_tiles: list[tuple[TileCoord, bytes]], bounds: GeoBounds, expand_zoom: int = 0
) -> CoverageReport:
    """
